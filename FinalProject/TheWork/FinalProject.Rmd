...
sim2018 <- sim2018 %>% 
  mutate(perc_Dem = sim_1, 
         chamdist = rep(1:120, times = 1),
         year = rep(2018)) %>%
  select(year, chamdist, perc_Dem)

Resultsfin$sim <- simulate(m8, re.form=NA, allow.new.levels = TRUE, newdata = Resultsfin)$sim_1
...
trend <- ggplot(Resultsfin) +
  geom_smooth(aes(y = perc_Dem, x = year))
trend
mean <- mean(Results2018$sim)

# trend is a time trend of perc_Dem
```
