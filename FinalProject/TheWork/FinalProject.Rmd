...
m4 <- lmer(perc_Dem ~
             perc_DemLag + incumbentpartyLag + incumbentparty + 
             (1|year/District) +
...
Calc_err <- function(formula = perc_Dem ~ (perc_prez_Dem + perc_gub_Dem), initial_lag = 20){
  n <- length(unique(Results1$year))
  n_predict <- n - initial_lag
  RSS <- rep(NA, n=predict)
  for(i in 1:n_predict){
    train <- Results1 %>%
      filter(year < min(Results1$year) + initial_lag + 2*i)
    test <- Results1 %>%
      filter(year == 2000)
    m1 <- lmer(formula = perc_Dem ~ (perc_change + Approve + (1|chamdist)), data = train)
...
Calc_err(perc_Dem ~ (perc_prez_Dem + perc_gub_Dem))
```
