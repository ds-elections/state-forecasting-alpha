...
```{r}
Calc_err <- function(formula, initial_lag = 20){
  ...
  for(i in 1:n_predict){
    ...
    test <- Results1 %>%
      filter(year == 2000)
    m1 <- lmer(perc_Dem ~ perc_change + (1|chamdist), data = train)
    
    y1 <- predict(m1 , data = test)
    y2 <- simulate(m1 , data = test)
    RSS[i] <- sum((test$perc_Dem - y2)^2)
  }
  mean(RSS)}

Calc_err(perc_Dem ~ perc_prez_Demt_1 + perc_prez_Demt_2)
```
